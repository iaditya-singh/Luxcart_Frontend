...
const Cart = (props: Props) => {
  ...
  function deleteCart(ev: React.MouseEvent<HTMLButtonElement>) {
    clearCart();
                        ...
                        <li className="flex items-center gap-4 justify-between">
                          ...
                          <div>
                            ...
                            <dl className="mt-0.5 space-y-px text-[10px] text-text">
                              <p className='text-lg text-primary'>$ {cartProducts.filter(id => id === product._id).length * parseFloat(product.price)}</p>
                            </dl>
                          </div>
